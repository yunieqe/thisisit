...
foreach ($file in $mmdFiles) {
    ...
    try {
        ...
        Write-Host "  Success: Successfully rendered $($file.Name)" -ForegroundColor Green
        $successCount++
        
    } catch {
        Write-Host "  Error: Failed to render $($file.Name): $($_.Exception.Message)" -ForegroundColor Red
        $errorCount++
...
if ($generatedFiles.Count -gt 0) {
    Write-Host ""
    Write-Host "Generated files in docs/diagrams/:" -ForegroundColor Cyan
    foreach ($generatedFile in $generatedFiles) {
        $sizeKB = [math]::Round($generatedFile.Length/1024, 2)
