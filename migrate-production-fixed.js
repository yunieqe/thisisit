...
/**
 * Migration Script: Fix Transaction Amounts (Production Version)
 * Uses DATABASE_URL environment variable for production database connection
 */

const { Pool } = require('pg');

console.log('🚀 ESCASHOP Production Migration - Fixed Version');
console.log('===============================================');

// Use DATABASE_URL if provided, otherwise fall back to individual config
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || `postgresql://${process.env.DB_USER || 'postgres'}:${process.env.DB_PASSWORD || 'password'}@${process.env.DB_HOST || 'localhost'}:${process.env.DB_PORT || '5432'}/${process.env.DB_NAME || 'escashop'}`
});
